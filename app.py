...
with st.expander("店舗の日付一覧ページからURLをまとめて取得する"):
    store_url = st.text_input("店舗の日付一覧ページURL", placeholder="https://min-repo.com/store/xxxxx/")
    period_months = st.radio("取得期間", options=[1, 2, 3], format_func=lambda x: f"{x}ヶ月", horizontal=True)
    ...
    if fetch_button and store_url:
        try:
            ...
            for a in soup_tmp.find_all('a', href=True):
                ...
                base_match = re.match(r'(https://min-repo\.com/\d+/?)', href)
                if not base_match:
                    continue
                clean_href = base_match.group(1)
                if not clean_href.endswith('/'):
                    clean_href += '/'
                text = a.get_text(strip=True)
                candidates.append((text, clean_href))
            # デバッグ：見つかったリンクを全表示
            all_links = [a['href'] for a in soup_tmp.find_all('a', href=True)]
            with st.expander("デバッグ情報（見つかった全リンク）"):
                st.text(f"全リンク数: {len(all_links)}")
                st.text('\n'.join(all_links[:50]))
            ...
            if not candidates:
                ...
                if not parsed:
                    ...
                    filtered_links = list(dict.fromkeys([h for d, h in parsed if d > cutoff]))
...
if run_button:
    urls = list(dict.fromkeys([u.strip() for u in url_text.strip().splitlines() if u.strip()]))
    if not urls:
    ...
    for i, url in enumerate(urls):
        ...
        for row in table.find_all('tr'):
            ...
            try:
                ...
                continue
            all_data.append({'日付': label, '機種名': kishu, '台数': total,
                             '勝率': win_rate_str, '勝率(%)': win_percent,
                             '平均差枚': avg_diff, '出玉率': deritsu})
    ...
    if repeat_all.empty:
        lines.append(f"今回の{total_dates}日分のデータでは、複数回登場した機種はありませんでした。")
    else:
        ...
        if high_rate:
            ...
            lines.append("登場した日は特に強く、積極的に狙う価値があります。")
            lines.append("")
        ...
        if avg_winrate_overall >= 70:
            ...
            lines.append(f"**総評：** 全体の平均勝率は{avg_winrate_overall}%です。複数回登場した機種を優先的に狙いましょう。")
    ...
    with tab1:
        ...
        st.download_button(label="CSVダウンロード（全日程一覧）",
                           data=display_df.to_csv(index=False, encoding='utf-8-sig'),
                           file_name="スロット分析_全日程.csv", mime="text/csv")
    ...
    with tab2:
        ...
        if repeat.empty:
            ...
            for kishu, cnt in repeat.items():
                ...
                summary_rows.append({'機種名': kishu, '登場回数': cnt,
                                     '平均勝率(%)': round(rows['勝率(%)'].mean(), 1),
                                     '設置台数': '/'.join(str(d) for d in rows['台数'].tolist()),
                                     '日付': ', '.join(rows['日付'].tolist())})
            summary_df = pd.DataFrame(summary_rows)
            st.dataframe(summary_df, use_container_width=True, height=600)
            st.download_button(label="CSVダウンロード（複数回登場まとめ）",
                               data=summary_df.to_csv(index=False, encoding='utf-8-sig'),
                               file_name="スロット分析_複数回登場.csv", mime="text/csv")
